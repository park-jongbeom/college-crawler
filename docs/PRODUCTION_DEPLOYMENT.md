...
운영 배포 시 `.env`는 GitHub Actions Secrets 기반으로 서버에서 생성되며, 최소 아래 값들이 필요합니다.

- DB 연결
  - `DATABASE_HOST`, `DATABASE_PORT`, `DATABASE_NAME`, `DATABASE_USER`, `DATABASE_PASSWORD`
- 감사로그 제약 대응(운영 DB에서 NOT NULL/FK 제약이 있는 경우)
  - `AUDIT_SYSTEM_USER_ID`
- Level 1 메타데이터 보강(College Scorecard)
  - `COLLEGE_SCORECARD_API`
  - 보안: 키는 Git에 커밋하지 않고, GitHub Secrets로만 관리합니다.
