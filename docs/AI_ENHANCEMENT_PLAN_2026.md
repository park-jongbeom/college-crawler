...
**대표님 주도**로 학교 담당자(국제학생 담당, 입학처 등)에게 직접 이메일을 전달하여 필요한 정보를 요청합니다.

**수집 대상 정보**:
- 웹사이트에 공개되지 않은 상세 데이터 (합격률, 편입률, 장학금 정책)
- 국제학생 전용 프로그램 정보 (ESL, 오리엔테이션, 취업 지원)
- 담당자 직통 연락처 (긴급 문의 시 활용)
- 최신 정책 변경사항 (학비, 입학 요건 등)

**프로세스**:
1. **이메일 발송**: 대표님이 84개 학교 담당자에게 정보 요청 이메일 전송
2. **응답 수집**: 학교별로 회신된 정보를 정리
3. **시스템 반영**: 기술팀이 수집된 정보를 DB에 입력 또는 크롤링 검증 데이터로 활용
4. **업데이트**: 직접 수집한 정보를 우선하여 웹 크롤링 데이터 보완

**장점**:
- 공식적이고 정확한 정보 확보 (1차 소스)
- 학교와 직접 관계 형성 (향후 파트너십 가능성)
- 크롤링으로 얻기 어려운 비공개 정보 수집
- 데이터 검증 (크롤링 결과와 대조)
